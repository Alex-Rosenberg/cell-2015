...
def value(w,*args):
	...
	V = sum(vv)/float(shape(X)[0]) - sum(w[:shape(X)[1]*shape(Y)[1]]**2)*reg_lambda
	G = np.array(Gobs-G_pred).transpose()
	G[:-1,:] -= reg_lambda*sign(w.reshape(shape(G))[:-1,:])
...
def MLR(X,Y,reg_lambda,w_init=None,maxfun=200,fixed_weights=None):
	...
	X: nxd matrix with n training cases and d features
	Y: nxk matrix with n training cases and k classes
